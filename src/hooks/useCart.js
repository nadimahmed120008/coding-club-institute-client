...
const useCart = () => {
  ...
  useEffect(() => {
    fetch(`https://scary-dracula-51446.herokuapp.com/cart/${uid}`)
      .then((res) => res.json())
      .then((data) => {
        if (data.length) {
  ...
  function addToCart(course) {
    ...
    if (isHave) {
      ...
      fetch("https://scary-dracula-51446.herokuapp.com/course/add", {
        method: "post",
  ...
  function remove(id) {
    fetch(`https://scary-dracula-51446.herokuapp.com/delete/${id}`, {
      method: "delete",
    })
      .then((res) => res.json())
      .then((data) => {
